import type { ParsedMethod, ParsedError, ParsedStruct, ParsedEnum, ContractMetadata } from './types';
...
export class RustContractParser {
  ...
  private parseMethodArgs(rawArgs: string): Array<{ name: string; type: string; optional: boolean }> {
    return rawArgs
      .split(',')
      .map(part => part.trim())
  ...
  private extractDocumentation(methodName: string): string | undefined {
    const docRegex = new RegExp(`///\\s*([^\\n]+)\\s*\\n[\\s\\S]*?pub fn ${methodName}`, 'g');
