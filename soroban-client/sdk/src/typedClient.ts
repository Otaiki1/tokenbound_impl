...
import type { ContractCallArtifact, InvokeOptions, WriteInvokeOptions } from './types';
...
export class TypedContractClient<TContract = unknown> {
  ...
  constructor(
    ...
    this.methodConfigs = new Map(methodConfigs.map(config => [config.name, config]));
  }
  ...
  protected async write(
    ...
    const config = this.methodConfigs.get(method);
    if (config?.isReadOnly) {
      throw new Error(`Method ${method} is a read-only method, use read() instead`);
    }
